...
void MAXreg_wr(BYTE reg, BYTE val) {
	//MAX3421E is a slave that we need to send data to
	//psuedocode:
	//select MAX3421E 
	XSpi_SetSlaveSelect(&SpiInstance,1); //the one means select the slave
	//write reg + 2 via SPI
	BYTE write_buff[] = {reg+2, val};
	//write val via SPI
	//read return code from SPI peripheral (see Xilinx examples) 
	int return_code = XSpi_Transfer(&SpiInstance, write_buff, NULL, 2);
	//if return code != 0 print an error
	if(return_code != 0){
		xil_printf("There's an error in MAXreg_wr with return type void\n");
	}
	//deselect MAX3421E (may not be necessary if you are using SPI peripheral)
	XSpi_SetSlaveSelect(&SpiInstance, 0); //deselects the slave
}
...
BYTE* MAXbytes_wr(BYTE reg, BYTE nbytes, BYTE* data) {
	...
	//write reg + 2 via SPI
	BYTE write_buff[nbytes+1];
	write_buff[0] = reg+2;
	//write data[n] via SPI, where n goes from 0 to nbytes-1
//	for(int i =0; i<nbytes; i++){
//		write_buff[i+1] = data[i];
//	}
	for(int i =1; i<=nbytes; i++){
		write_buff[i] = data[i-1];
	}
	//read return code from SPI peripheral 
	int return_code = XSpi_Transfer(&SpiInstance, write_buff, NULL, nbytes+1);
	//if return code != 0 print an error
	if(return_code != 0){
		xil_printf("There's an error in Maxbytes_wr with return type BYTE*\n");
	}
	//deselect MAX3421E (may not be necessary if you are using SPI peripheral)
...
BYTE MAXreg_rd(BYTE reg) {
	...
	BYTE write_buf[2];
	write_buf[0] = reg;
	BYTE read_buf[2]; //this is two bits since the send and receive buffer need to be the same length
	//read val via SPI
	int return_code = XSpi_Transfer(&SpiInstance, write_buf, read_buf, 2);
	//read return code from SPI peripheral 
	//if return code != 0 print an error
	if(return_code != 0){
		xil_printf("Error in Maxreg_rd with return type BYTE\n");
	}
	//deselect MAX3421E (may not be necessary if you are using SPI peripheral)
	XSpi_SetSlaveSelect(&SpiInstance, 0);
	//return val
...
BYTE* MAXbytes_rd(BYTE reg, BYTE nbytes, BYTE* data) {
	...
	XSpi_SetSlaveSelect(&SpiInstance, 1);
	//write reg via SPI
	BYTE write_buf[nbytes+1];
	write_buf[0] = reg;
	BYTE read_buf[nbytes+1];

	int return_code = XSpi_Transfer(&SpiInstance, write_buf, read_buf, nbytes+1);
	if(return_code != 0){
		xil_printf("Error in MAXbytes_rd with return type BYTE*\n");
	}
